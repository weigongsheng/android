...
public class PullToRefreshLayout extends RelativeLayout implements
		OnTouchListener,RefreshResultNotify {
	...
	// 刷新失败
	public static final int REFRESH_ABORT = 2;
	// 下拉头
	...
	private int headHeith;
	private int headWidth;
	private int contentWidth;
	private int contentHeith;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (!isLayout) {
			...
			refreshDist = ((ViewGroup) headView).getChildAt(0)
					.getMeasuredHeight();
			headHeith = headView.getMeasuredHeight();
			headWidth = headView.getMeasuredWidth();
			contentWidth = contentView.getMeasuredWidth();
			contentHeith = contentView.getMeasuredHeight();
		}
		if (canPull ) {
			// 改变子控件的布局
			headView.setVisibility(View.VISIBLE);
			headView.layout(0, (int) moveDeltaY - headHeith,
					headWidth, (int) moveDeltaY);
			contentView.layout(0, (int) moveDeltaY,contentWidth, (int) moveDeltaY + contentHeith);
		} 
		else{
