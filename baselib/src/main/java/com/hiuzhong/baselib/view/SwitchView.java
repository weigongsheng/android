...
public class SwitchView extends View {
	...
	private interface OnInnerStateChangedListener {
		void toggleToOn();

		void toggleToOff();
	}

	public interface OnStateChangedListenersss{
		void onChanged(boolean switchOn);
	}

	private OnInnerStateChangedListener listener = new OnInnerStateChangedListener() {
		@Override public void toggleToOn() {
			toggleSwitch(STATE_SWITCH_ON);
			if(outerListener != null){
				outerListener.onChanged(true);
			}
		}
		...
		@Override public void toggleToOff() {
			...
			if(outerListener != null){
				outerListener.onChanged(false);
			}
		}
	};

	private OnStateChangedListenersss outerListener;

	public void setOnStateChangedListener(OnStateChangedListenersss listener) {
		this.outerListener = listener;
	}

	public boolean isOn(){
		return getState() == STATE_SWITCH_ON;
	}
}